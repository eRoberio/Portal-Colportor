...
class _HomePageState extends ConsumerState<HomePage> {
  ...
  String _abaAtual = 'estudante'; // Controla se estamos vendo Efetivos ou Estudantes
...
// Função que abre a galeria e envia a foto
  ...
  Future<void> _selecionarDataNascimento(String uid) async {
    ...
    if (dataEscolhida != null) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text('🎉 Data salva com sucesso!'), backgroundColor: Colors.green),
        );
      }
    }
  }
...
Widget _buildInicioTab(Map<String, dynamic>? profile) {
    ...
    final totalStatsAsync = uid != null ? ref.watch(userTotalStatsProvider(uid)) : const AsyncValue.loading();
        ...
        children: [
          ...
          if (needsPhoto && uid != null)
            Container(
              ...
              decoration: BoxDecoration(color: Colors.red.shade50, border: Border.all(color: Colors.red.shade200, width: 2), borderRadius: BorderRadius.circular(16)),
              child: Row(
                children: [
                  ...
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('Adicione uma foto de perfil.', style: GoogleFonts.inter(fontSize: 12, color: Colors.red.shade900)),
                      ],
                    ),
                  ),
                  const SizedBox(width: 8),
                  ElevatedButton(
                    onPressed: () => _selecionarDataNascimento(uid), // ou _pickAndUploadImage
          ...
          _buildActionCard(
            icon: '📝', title: 'Novo Relatório', subtitle: 'Envie seu relatório diário', 
            onTap: () { if (uid != null) Navigator.push(context, MaterialPageRoute(builder: (context) => NovoRelatorioPage(uid: uid))); }
          ),
          _buildActionCard(
            icon: '🏆', title: 'Rankings', subtitle: 'Veja os campeões do período', 
            onTap: () { Navigator.push(context, MaterialPageRoute(builder: (context) => RankingsPage( isAdmin: isAdmin,minhaCategoria: profile?['categoria'] ?? 'estudante',)));
            },
          ),
          _buildActionCard(
            icon: '🎯', title: 'Desafios', subtitle: 'Confira os desafios ativos', 
            onTap: () { if (uid != null) Navigator.push(context, MaterialPageRoute(builder: (context) => DesafiosPage(uid: uid))); }
          ),
          if (isAdmin)
            _buildActionCard(
              icon: '⚙️', title: 'Painel Admin', subtitle: 'Aprovar colportores e gerar códigos', 
         ...
         // ===========================
              ...
              return Column(
                children: [
                  ...
                  if (isAdmin) ...[
                    Row(
                      children: [
                        Expanded(child: _buildTabButton('👥 Efetivos', 'efetivo')),
                        const SizedBox(width: 8),
                        Expanded(child: _buildTabButton('🎓 Estudantes', 'estudante')),
                      ],
                    ),
                  ...
                  championsAsync.when(
                    ...
                    error: (err, stack) => Text('Erro ao carregar campeões.', style: GoogleFonts.inter(color: Colors.red)),
                    data: (todaysList) {
                      return usersMapAsync.when(
                        ...
                        data: (users) {
                          ...
                          final filteredList = todaysList.where((stats) {
                            ...
                            final userCategoria = users[userId]?['categoria'] ?? 'estudante'; 
                          ...
                          if (filteredList.isEmpty) {
                            ...
                            final nomeAbaFormatado = categoriaParaExibir == 'efetivo' ? 'Efetivos' : 'Estudantes';
                            ...
                            return Container(
                              ...
                              child: Column(
                                children: [
                                  Icon(LucideIcons.medal, size: 40, color: Colors.grey[300]),
                                  const SizedBox(height: 8),
                                  Text('Nenhum relatório de $nomeAbaFormatado hoje.', style: GoogleFonts.inter(color: Colors.grey[600])),
                                  Text('Seja o primeiro a pontuar!', style: GoogleFonts.inter(fontWeight: FontWeight.bold, color: const Color(0xFF1E3A8A))),
                                ],
                              ),
                            );
                          }
                          ...
                          for (var stats in filteredList) {
                            if (topHoras == null || stats['horas'] > topHoras['horas']) topHoras = stats;
                            if (topOfertas == null || stats['ofertas'] > topOfertas['ofertas']) topOfertas = stats;
                            if (topLivros == null || stats['livros'] > topLivros['livros']) topLivros = stats;
                          }

                          return Column(
                            children: [
                              if (topHoras != null && topHoras['horas'] > 0)
                                _buildChampionCard(title: '⏰ Horas Missionárias', icon: '📚', name: users[topHoras['uid']]?['nome'] ?? 'Colportor', score: '${topHoras['horas']}h', isGold: true),
                              if (topOfertas != null && topOfertas['ofertas'] > 0)
                                _buildChampionCard(title: '💰 Ofertas', icon: '💰', name: users[topOfertas['uid']]?['nome'] ?? 'Colportor', score: 'R\$ ${topOfertas['ofertas'].toStringAsFixed(2).replaceAll('.', ',')}', isGold: false),
                              if (topLivros != null && topLivros['livros'] > 0)
                                _buildChampionCard(title: '💼 Livros', icon: '💼', name: users[topLivros['uid']]?['nome'] ?? 'Colportor', score: '${topLivros['livros']} un', isGold: false),
                            ],
                          );
                        },
                      );
          ...
          Container(
            ...
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text('📊 Minhas Estatísticas', style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.bold, color: const Color(0xFF333333))),
                const SizedBox(height: 16),
                
                totalStatsAsync.when(
                  loading: () => const Center(child: Padding(padding: EdgeInsets.all(20), child: CircularProgressIndicator())),
                  error: (err, stack) => Text('Erro ao carregar estatísticas.', style: GoogleFonts.inter(color: Colors.red)),
                  data: (stats) {
                    return Column(
                      children: [
                        Row(
                          children: [
                            Expanded(child: _buildStatBox('Horas', stats['horas'].toStringAsFixed(1))),
                            const SizedBox(width: 8),
                            Expanded(child: _buildStatBox('Orações', stats['oracoes'].toString())),
                          ],
                        ),
                        const SizedBox(height: 8),
                        Row(
                          children: [
                            Expanded(child: _buildStatBox('Ofertas', "R\$ ${stats['ofertas'].toStringAsFixed(2).replaceAll('.', ',')}")),
                            const SizedBox(width: 8),
                            Expanded(child: _buildStatBox('Livros', stats['livros'].toString())),
                          ],
                        ),
                      ],
                    );
...
// ==========================================
  ...
  Widget _buildRelatoriosTab(String? uid) {
    if (uid == null) return const Center(child: Text('Usuário não identificado.'));
      ...
      data: (snapshot) {
          ...
          return Center(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                Text('Nenhum relatório enviado ainda.', style: GoogleFonts.inter(fontSize: 16, color: Colors.grey[600])),
                const SizedBox(height: 8),
                Text('Vá no Início e clique em "Novo Relatório".', style: GoogleFonts.inter(fontSize: 14, color: Colors.grey[500])),
              ],
            ),
          );
        }
        ...
        return ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            final dataFormatada = '${date.day.toString().padLeft(2, '0')}/${date.month.toString().padLeft(2, '0')}/${date.year}';
                ...
                child: Column(
                  ...
                  children: [
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        Text('📅 $dataFormatada', style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 16, color: const Color(0xFF1E3A8A))),
                        Container(
                          padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                          ...
                          child: Text('Enviado', style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.green.shade700)),
                        ),
                      ],
                    ),
                    ...
                    Row(
                      mainAxisAlignment: MainAxisAlignment.spaceBetween,
                      children: [
                        _buildMiniStat('⏰', '${report['horas']}h'),
                        _buildMiniStat('📚', '${report['livros']} un'),
                        _buildMiniStat('💰', 'R\$ ${report['ofertas']}'),
                        _buildMiniStat('🙏', '${report['oracoes']}'),
                      ],
                    ),
  ...
  Widget _buildAniversariosTab(Map<String, dynamic>? profile) {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('🎂 Aniversariantes', style: GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.bold, color: const Color(0xFF1E3A8A))),
          Text('Celebre a vida com a equipe em ${nomesMeses[mesAtual]}', style: GoogleFonts.inter(fontSize: 14, color: Colors.grey[600])),
          const SizedBox(height: 24),
          ...
          if (precisaPreencherData)
            Container(
              ...
              child: Row(
                children: [
                  const Icon(LucideIcons.gift, color: Color(0xFFF59E0B), size: 32),
                  const SizedBox(width: 16),
                  Expanded(
                    child: Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('Quando é o seu?', style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 15, color: Colors.black87)),
                        const SizedBox(height: 4),
                        Text('Adicione sua data para a equipe celebrar com você.', style: GoogleFonts.inter(fontSize: 12, color: Colors.grey[700])),
                      ],
                    ),
                  ),
                  const SizedBox(width: 8),
          ...
          usersMapAsync.when(
            ...
            error: (err, stack) => Text('Erro ao carregar dados.', style: GoogleFonts.inter(color: Colors.red)),
            data: (usersMap) {
              
              // Filtra apenas quem faz aniversário neste mês
              final aniversariantes = usersMap.values.where((u) => u['mes_nascimento'] == mesAtual).toList();
              
              // Ordena pelo dia do aniversário (do dia 1 ao 31)
              aniversariantes.sort((a, b) => (a['dia_nascimento'] as int).compareTo(b['dia_nascimento'] as int));
              ...
              if (aniversariantes.isEmpty) {
                return Center(
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        Icon(LucideIcons.calendarX2, size: 60, color: Colors.grey[300]),
                        const SizedBox(height: 16),
                        Text('Nenhum aniversário este mês.', style: GoogleFonts.inter(fontSize: 16, color: Colors.grey[600])),
                      ],
                    ),
                  ),
                );
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, index) {
                  ...
                  final telefone = user['telefone']; // Caso queira usar para um botão de WhatsApp no futuro
                  ...
                  return Card(
                    ...
                    shape: RoundedRectangleBorder(
                      borderRadius: BorderRadius.circular(12),
                      side: isHoje ? const BorderSide(color: Color(0xFF10B981), width: 2) : BorderSide.none,
                    ),
                    child: ListTile(
                      contentPadding: const EdgeInsets.symmetric(horizontal: 16, vertical: 8),
                      leading: CircleAvatar(
                        radius: 24,
                        backgroundImage: (fotoUrl != null && fotoUrl.toString().isNotEmpty) ? NetworkImage(fotoUrl) : null,
                        backgroundColor: isHoje ? const Color(0xFF10B981) : const Color(0xFF1E3A8A),
                        child: (fotoUrl == null || fotoUrl.toString().isEmpty)
                            ? Text(nome[0].toUpperCase(), style: const TextStyle(color: Colors.white, fontSize: 18))
                            : null,
                      ),
                      title: Row(
                        children: [
                      ...
                      subtitle: Text(
                        ...
                        style: GoogleFonts.inter(
                          color: isHoje ? const Color(0xFF10B981) : Colors.grey[600],
                          fontWeight: isHoje ? FontWeight.bold : FontWeight.normal,
                        ),
                      ),
                      trailing: IconButton(
                        icon: const Icon(LucideIcons.messageCircle, color: Color(0xFF25D366)), // Cor do WhatsApp
                        onPressed: () {
                          // Lógica futura: Abrir url_launcher com "https://wa.me/55${telefone limpo}"
                          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Integração com WhatsApp em breve!')));
                        },
                      ),
                    ),
                  );
 ...
 // ==========================================
  ...
  Widget _buildMotivacaoTab() {
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Text('📖 Guia de Bolso', style: GoogleFonts.inter(fontSize: 24, fontWeight: FontWeight.bold, color: const Color(0xFF1E3A8A))),
          Text('O Colportor Evangelista (E. G. White)', style: GoogleFonts.inter(fontSize: 14, color: Colors.grey[600])),
          const SizedBox(height: 24),
          ...
          Container(
            ...
            decoration: BoxDecoration(
              ...
              borderRadius: BorderRadius.circular(20),
              boxShadow: [BoxShadow(color: const Color(0xFF8B5CF6).withValues(alpha: 0.3), blurRadius: 15, offset: const Offset(0, 8))],
            ),
            child: Column(
              children: [
                ...
                const SizedBox(height: 24),
                TextButton.icon(
                  onPressed: () => setState(() {}), // Apenas recarrega a tela para sortear nova frase
                  icon: const Icon(LucideIcons.refreshCw, color: Colors.white, size: 18),
                  label: Text('Sorteio Aleatório', style: GoogleFonts.inter(color: Colors.white, fontWeight: FontWeight.bold)),
          ...
          Text('🎯 O que você precisa agora?', style: GoogleFonts.inter(fontSize: 18, fontWeight: FontWeight.bold, color: const Color(0xFF333333))),
          const SizedBox(height: 16),
          ...
          ...EgwDatabase.categorias.keys.map((categoria) {
            ...
            if (categoria.contains('Consagração')) { icone = LucideIcons.heartHandshake; cor = const Color(0xFF10B981); }
            else if (categoria.contains('Abordagem')) { icone = LucideIcons.doorOpen; cor = const Color(0xFFF59E0B); }
            else if (categoria.contains('Dificuldades')) { icone = LucideIcons.shieldAlert; cor = const Color(0xFFEF4444); }
  ...
  Widget _buildTemaCard(String categoria, IconData icon, Color color) {
    return Card(
      ...
      child: ListTile(
        ...
        title: Text(categoria, style: GoogleFonts.inter(fontWeight: FontWeight.bold, fontSize: 16)),
        trailing: const Icon(LucideIcons.chevronRight, color: Colors.grey),
            ...
            return Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  const SizedBox(height: 24),
                  Text(categoria, style: GoogleFonts.inter(fontSize: 20, fontWeight: FontWeight.bold, color: cor)),
                  const SizedBox(height: 24),
                  Text(
                    '"$frase"',
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton.icon(
                      ...
                      label: Text('Ler mais sobre isso', style: GoogleFonts.inter(fontWeight: FontWeight.bold)),
                      style: ElevatedButton.styleFrom(
                        backgroundColor: cor,
                        foregroundColor: Colors.white,
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12)),
  ...
  Widget _buildTrainingCard({required String title, required String subtitle, required IconData icon, required Color color}) {
    return Card(
      color: Colors.white,
      elevation: 1,
      margin: const EdgeInsets.only(bottom: 12),
      ...
      child: ListTile(
        ...
        subtitle: Text(subtitle, style: GoogleFonts.inter(color: Colors.grey[600], fontSize: 13)),
        trailing: const Icon(LucideIcons.chevronRight, color: Colors.grey),
        onTap: () {
          // Lógica futura: Abrir um vídeo do YouTube ou um PDF
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Material sendo preparado pela Liderança!')));
        },
      ),
    );
  }
